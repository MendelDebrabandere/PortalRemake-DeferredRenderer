#include "stdafx.h"
#include "PortalMapRenderer.h"
#include "Misc/PortalMapMaterial.h"
...
void PortalMapRenderer::Initialize()
{
	...
	m_pPortalRenderTarget = new RenderTarget(m_GameContext.d3dContext);
	...
	HRESULT result = m_pPortalRenderTarget->Create(rtDesc);

	if (FAILED(result))
	{
		std::cout << "failed to make rendertarget in PortalMapRenderer.cpp\n";
	}
	...
	m_pPortalMapGenerator = new PortalMapMaterial();
	m_pPortalMapGenerator->Initialize(m_GameContext.d3dContext, 0);

	m_GeneratorTechniqueContext = m_pPortalMapGenerator->GetTechniqueContext(0);
}

void PortalMapRenderer::UpdateMeshFilter(const SceneContext& sceneContext, MeshFilter* pMeshFilter) const
{
	...
	const MaterialTechniqueContext& techniqueContext = m_GeneratorTechniqueContext;
...
void PortalMapRenderer::Begin(const SceneContext& sceneContext)
{
	...
	const auto& rtDesc = m_pPortalRenderTarget->GetDesc();
	...
	m_pPortalMapGenerator->SetVariable_Matrix(L"gLightViewProj", m_LightVP);
	...
	m_GameContext.pGame->SetRenderTarget(m_pPortalRenderTarget);

	//5. Clear the ShadowMap rendertarget (RenderTarget::Clear)
	m_pPortalRenderTarget->Clear();
}

void PortalMapRenderer::DrawMesh(const SceneContext& sceneContext, MeshFilter* pMeshFilter, const XMFLOAT4X4& meshWorld, const std::vector<XMFLOAT4X4>& /*meshBones*/)
{
	...
	const MaterialTechniqueContext& curTechniqueContext{ m_GeneratorTechniqueContext };
	m_pPortalMapGenerator->SetVariable_Matrix(L"gWorld", reinterpret_cast<const float*>(&meshWorld));
	...
	for (const auto& subMesh : pMeshFilter->GetMeshes())
	{
		...
		m_pPortalMapGenerator->SetTechnique(0);
...
void PortalMapRenderer::End(const SceneContext&) const
{
...
ID3D11ShaderResourceView* PortalMapRenderer::GetShadowMap() const
{
	return m_pPortalRenderTarget->GetDepthShaderResourceView();
}

void PortalMapRenderer::Debug_DrawDepthSRV(const XMFLOAT2& position, const XMFLOAT2& scale, const XMFLOAT2& pivot) const
{
	if (m_pPortalRenderTarget->HasDepthSRV())
	{
		SpriteRenderer::Get()->DrawImmediate(m_GameContext.d3dContext, m_pPortalRenderTarget->GetDepthShaderResourceView(), position, XMFLOAT4{ Colors::White }, pivot, scale);
